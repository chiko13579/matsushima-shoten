...
const CONFIG = {
  baseUrl: 'https://imitsu.jp/ct-notary-public/search/',
  outputFile: '全国の行政書士リスト_imitsu.csv',
  progressFile: '全国の行政書士リスト_imitsu_進行中.csv',
  delayBetweenPages: 2000,
  delayBetweenDetails: 1500,
  resumeFromExisting: true,
  totalPages: 93, // 1,846件 ÷ 20件/ページ ≈ 93ページ
...
async function scrapeListPage(page, pageNum) {
  try {
    ...
    const results = await page.evaluate(() => {
      ...
      // h3タグ内のリンクを全て取得
      const links = document.querySelectorAll('h3 a[href*="/ct-notary-public/"]');
 ...
 */
async function main() {
  console.log('🚀 行政書士リスト取得を開始します（imitsu.jp）\n');
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= CONFIG.totalPages; pageNum++) {
      ...
      console.log(`  ✓ ${listItems.length}件の行政書士を発見`);
