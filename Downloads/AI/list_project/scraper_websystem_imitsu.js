...
const CONFIG = {
  baseUrl: 'https://imitsu.jp/ct-web-system/search/',
  outputFile: '全国のWebシステム開発会社リスト_imitsu.csv',
  progressFile: '全国のWebシステム開発会社リスト_imitsu_進行中.csv',
  delayBetweenPages: 2000,
  delayBetweenDetails: 1500,
  resumeFromExisting: true,
  totalPages: 515, // 10,285件 ÷ 20件/ページ ≈ 515ページ
...
async function scrapeListPage(page, pageNum) {
  try {
    ...
    const results = await page.evaluate(() => {
      ...
      // h3タグ内のリンクを全て取得
      const links = document.querySelectorAll('h3 a[href*="/ct-web-system/"]');
 ...
 */
async function main() {
  console.log('🚀 Webシステム開発会社リスト取得を開始します（imitsu.jp）\n');
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= CONFIG.totalPages; pageNum++) {
      ...
      console.log(`  ✓ ${listItems.length}件のWebシステム開発会社を発見`);
