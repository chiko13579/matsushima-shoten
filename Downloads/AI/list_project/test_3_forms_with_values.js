...
const FORMS_TO_TEST = [
  ...
  { name: '税理士法人アレックスパートナーズ', url: 'https://www.toshikaikei.biz/call/' },
  { name: '横浜中央税理士法人', url: 'https://www.yckz.co.jp/contact' },
  ...
  { name: '税理士法人関内会計', url: 'https://www.kannaikaikei.jp/contact' },
  { name: '川村会計事務所', url: 'https://www.kawamura-tax.jp/contact' }
];
...
async function testForm(form, index) {
  ...
  try {
    ...
    const result = await page.evaluate((SENDER_INFO) => {
      ...
      const nameFieldGroups = {}; // 同じラベルの名前フィールドをグループ化
      ...
      function detectFieldType(element) {
        ...
        // 電話番号パターン（例：03-3391-1044, 090-1234-5678）で検出
        if (allText.match(/\d{2,4}[-ー]\d{2,4}[-ー]\d{3,4}/)) {
        ...
        // 問い合わせ内容（TEXTAREAのみ、または特定ラベル）- SELECT/チェックボックス/ラジオは除外
        // 「種別」「タイプ」「カテゴリ」などはSELECT用のラベルなので除外
        if (element.tagName === 'TEXTAREA' ||
            (allText.match(/問い合わせ内容|お問合せ内容|ご相談内容|相談内容|質問内容|ご質問|詳細|備考|要件|用件|message|inquiry|comment|detail/) &&
      ...
      elements.forEach(element => {
        ...
        // 常に入力するフィールドタイプ（名前、メール、会社名、電話、住所、お問合せ内容、同意）
        const alwaysFillTypes = [
          'email', 'lastName', 'firstName', 'fullName', 'lastNameKana', 'firstNameKana', 'fullNameKana', 'message',
          'company', 'tel', 'tel1', 'tel2', 'tel3', 'address', 'prefecture', 'city', 'street', 'cityAndStreet', 'addressWithoutPref',
          'zip', 'zip1', 'zip2', 'agreement'
        ];
        ...
        try {
          if (detection.type === 'prefecture' || detection.type === 'city' || detection.type === 'cityAndStreet' || detection.type === 'address' || detection.type === 'addressWithoutPref') {
            ...
            // 非表示のチェックボックスの場合、関連するクリック可能要素をクリック
      ...
      // 最後に全ての必須項目をチェック：selectとradioで未選択のものに「その他」または最初の有効な選択肢を設定
      elements.forEach(element => {
        if (element.type === 'hidden' || element.type === 'submit' || element.type === 'button') return;
        ...
        const isRequired = element.required ||
                          element.hasAttribute('required') ||
                          element.getAttribute('aria-required') === 'true' ||
                          (element.label && (element.label.includes('*') || element.label.includes('＊') || element.label.includes('※') || element.label.includes('必須') || element.label.includes('Required')));

        // selectボックスで必須かつ未選択の場合
        if (isRequired && element.tagName.toLowerCase() === 'select') {
          const currentValue = element.value;
          const currentText = element.options[element.selectedIndex]?.textContent.trim() || '';

          // 未選択または「選択してください」系の場合
          if (!currentValue || currentText.match(/選択|select|choose|--/i)) {
            const options = element.options;
            let selectedIndex = -1;
            let foundOther = false;
            ...
            if (selectedIndex !== -1) {
              ...
              element.dispatchEvent(new Event('change', { bubbles: true }));
              const selectedText = options[selectedIndex].textContent.trim();
              filledFields.push({ type: 'final-check-select', label: element.label || element.name, value: selectedText });
            }
          }
        }

        // radioボタンで必須かつ未選択の場合
        if (isRequired && element.type === 'radio') {
          const radioName = element.name;
          const radioGroup = document.querySelectorAll(`input[type="radio"][name="${radioName}"]`);
          ...
          if (!alreadyChecked) {
            ...
            radioGroup.forEach(radio => {
              ...
              const radioValue = radio.value || '';
              const allText = `${radioLabel} ${radioValue}`;
            ...
            if (selectedRadio) {
              ...
              filledFields.push({ type: 'final-check-radio', label: element.label || radioName, value: radioLabel || selectedRadio.value || 'checked' });
            }
          }
        }

        // チェックボックスで必須かつ未選択の場合（TH要素のラベルも確認）
        if (element.type === 'checkbox') {
          ...
          // TH要素のラベルで必須かどうかも判定
          const isRequiredByTh = thLabel.includes('*') || thLabel.includes('＊') || thLabel.includes('※') || thLabel.includes('必須') || thLabel.includes('Required');

          // 必須の場合のみ処理
          if (isRequired || isRequiredByTh) {
            const checkboxName = element.name;
            const checkboxGroup = document.querySelectorAll(`input[type="checkbox"][name="${checkboxName}"]`);
            ...
            // まだチェックされていない場合、かつ複数のチェックボックスがある場合
            if (!alreadyChecked && checkboxGroup.length > 1) {
              let selectedCheckbox = null;
              let foundOther = false;
              ...
              if (selectedCheckbox) {
                ...
                if (!cbLabel) {
                  const parentLabel = selectedCheckbox.closest('label');
                  if (parentLabel) cbLabel = parentLabel.textContent.trim();
                }

                filledFields.push({ type: 'final-check-checkbox', label: element.label || checkboxName, value: cbLabel || selectedCheckbox.value || 'checked' });
              }
            }
          }
        }
